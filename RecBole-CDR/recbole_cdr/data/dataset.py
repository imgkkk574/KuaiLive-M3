...
class CrossDomainSingleDataset(Dataset):
    ...
    def _load_feat(self, filepath, source):
        """Override to use engine='c' instead of 'python' for fast CSV loading.
        RecBole hardcodes engine='python'; on 49M-row files this is prohibitively slow.
        engine='c' is safe for simple tab-separated files with no regex separators.
        """
        import pandas as pd
        _orig = pd.read_csv
        def _c_engine(*args, **kwargs):
            if kwargs.get('engine') == 'python':
                kwargs['engine'] = 'c'
            return _orig(*args, **kwargs)
        pd.read_csv = _c_engine
        try:
            n_rows_before = 0
            print(f"  [load_feat] {filepath.split('/')[-1]} ({self.domain}) ...", flush=True)
            t0 = time.time()
            result = super()._load_feat(filepath, source)
            print(f"  [load_feat] done  ({time.time()-t0:.1f}s)", flush=True)
        finally:
            pd.read_csv = _orig
        return result
    ...
    def _remap_fields(self, field_names, map_dict):
        ...
        """
        # Flatten ChainMap to a plain dict so pandas .map() uses its fast
        # Cython path instead of calling a Python lambda 49M+ times.
        flat = dict(map_dict) if hasattr(map_dict, 'maps') else map_dict

        def _fast_remap(series):
            mapped = series.map(flat)
        ...
        for field_name in field_names:
            ...
            if field_name in self.inter_feat.columns:
                n = len(self.inter_feat)
                t0 = time.time()
                print(f"  [remap] {field_name}  inter_feat {n:,} rows ...", flush=True)
                self.inter_feat[field_name] = _fast_remap(self.inter_feat[field_name])
                print(f"  [remap] {field_name}  done  ({time.time()-t0:.1f}s)", flush=True)
            if self.user_feat is not None and field_name in self.user_feat.columns:
                self.user_feat[field_name] = _fast_remap(self.user_feat[field_name])
            if self.item_feat is not None and field_name in self.item_feat.columns:
                self.item_feat[field_name] = _fast_remap(self.item_feat[field_name])
    ...
    def split_train_valid(self):
        ...
        if ordering_args == 'RO':
            ...
            if self.time_field is not None:
                ...
                self.shuffle()  # no TIME_FIELD in this domain, fall back to random
        else:
...
class CrossDomainDataset:
    ...
    def __init__(self, config):
        ...
        # token link remap
        # Wrap plain dicts in ChainMap so get_keys_from_chainmap_by_order works
        link_uid = ChainMap(self.user_link_dict) if isinstance(self.user_link_dict, dict) else self.user_link_dict
        link_iid = ChainMap(self.item_link_dict) if isinstance(self.item_link_dict, dict) else self.item_link_dict
        self.source_domain_dataset.remap_user_item_id(link_uid, link_iid)
