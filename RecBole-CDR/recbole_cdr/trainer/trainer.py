...
class DisenCDRTrainer(CrossDomainTrainer):
    r"""Trainer for DisenCDR.

    Adds a cross-phase global epoch counter so the model can gate its warm-up
    phase (specific-only, KLD disabled) on ``self._global_epoch`` rather than
    the per-phase ``epoch_idx``. Everything else is inherited from
    :class:`CrossDomainTrainer`.
    """

    def __init__(self, config, model):
        super(DisenCDRTrainer, self).__init__(config, model)
        self._disen_global_epoch = 0

    def _train_epoch(self, train_data, epoch_idx, loss_func=None, show_progress=False):
        # Advance the model's global epoch counter before training this epoch so
        # calculate_loss() can decide warm-up vs. full disentanglement.
        if hasattr(self.model, 'set_epoch'):
            self.model.set_epoch(self._disen_global_epoch)
        result = super(DisenCDRTrainer, self)._train_epoch(
            train_data, epoch_idx, loss_func=loss_func, show_progress=show_progress)
        self._disen_global_epoch += 1
        return result
