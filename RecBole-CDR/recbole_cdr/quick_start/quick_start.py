...
def run_recbole_cdr(model=None, config_file_list=None, config_dict=None, saved=True):
    ...
    try:
        ...
        emb_size = '(model-specific)'
    logger.info(
        '\n' + '=' * 60 + '\n'
        f"  Model   : {config['model']}\n"
        f"  Phases  : {config['train_epochs']}\n"
        f"  Users   : {ds.num_total_user}  (overlap: {ds.num_overlap_user - 1})\n"
        f"  Items   : {ds.num_total_item}  (overlap: {ds.num_overlap_item - 1})\n"
